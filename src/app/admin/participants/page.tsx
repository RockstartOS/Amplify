      ...
      <AdminHeader
        ...
        action={
          ...
          <a
            ...
            className="rounded-full border border-ink/15 px-4 py-2 text-sm font-semibold transition-colors hover:bg-surface"
          >
            ↓ Export CSV
          </a>
        }
              ...
              <tr key={r.id} className="border-b border-ink/10 last:border-0">
                ...
                <td className="px-5 py-3">
                  <form action={updateRegistrationStatusAction} className="flex items-center gap-2">
                    ...
                    <span className={`mono rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.16em] ${statusStyles[r.status] ?? "border-ink/15 bg-ink/5 text-ink/60"}`}>
                      {r.status}
                    </span>
                    <select name="status" defaultValue={r.status} className="rounded border border-ink/15 bg-transparent px-1.5 py-1 text-xs outline-none focus:border-brand">
